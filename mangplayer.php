...
$update = false;
 ...
 {
  $srno = $_GET['delete'];
  $sql = "DELETE FROM `players` WHERE player_id = $srno";
 ...
 { 
  ...
  $contact=$_POST['editcontact'];
   $sql="UPDATE `players` SET `player_name` = '$player_name', `email` = '$email', `player_type` = '$player_type', `game_type` = '$game_type', `address` = '$address', `contact` = '$contact' WHERE `players`.`player_id` = $srno;";
   $result = mysqli_query($conn,$sql);
   if($result)
   {
     $update = true;
            ...
            <tbody>
                  ...
                  if(mysqli_num_rows($result)>0)
                  {
                    foreach($result as $row)
                    {
                     ...
                     <td><img src="<?php echo "upload-images/".$row['player_img'];?>" alt="" width="100px" ></td>
                      ...
                      <button class='delete btn btn-sm btn-danger' id="d <?php echo $row['player_id']?> ">Delete</button></td>
                          </tr>
                          <?php
                    }
                  }
...
<script>
 ...
 Array.from(deletes).forEach((element)=>{
  element.addEventListener("click",(e)=>{
    ...
    {
      console.log("yes");
      window.location = `mangplayer.php?delete=${srno}`;
    }
    else
    {
      console.log("no");
